...
if($event_title == ""){//check if inputs have void event name
    ...
    $stmt = $mysqli->prepare("INSERT INTO events (username, event_title, event_date, event_time, is_urgent) VALUES (?, ?, ?, ?, ?)");
	if(!$stmt){
        echo json_encode(array(
            "success" => false,
	...
	$stmt->bind_param('sssss', $username, $event_title, $date, $time, $urgent);
	$stmt->execute();
    $stmt->close();
    echo json_encode(array(
        "success" => true,
